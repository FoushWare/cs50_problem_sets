...
const char* lookup(const char* path)
{
    char*extension=strrchr(path,'.'); // return pointer to the last occurance of the
    if(path!=NULL)
    {
    //Return text/css for any file whose path ends in .css (or any capitalization thereof)
    if(strcasecmp(extension,".css")==0)
    return "text/css";
   
    //Return text/html for any file whose path ends in .html (or any capitalization thereof)
    if(strcasecmp(extension,".html")==0)
    return "text/html";
    //Return image/gif for any file whose path ends in .gif (or any capitalization thereof)
     if(strcasecmp(extension,".gif")==0)
    return "image/gif";
    //Retrun image/x-icon for any file whose path ends in .ico (or any capitalization thereof)
     if(strcasecmp(extension,".ico")==0)
    return "image/x-icon";
    //Return image/jpeg (not image/jpg) for any file whose path ends in .jpg (or any capitalization thereof)
     if(strcasecmp(extension,".jpg")==0)
    return "image/jpeg";
    //Retrun text/javascript for any file whose path ends in .js (or any capitalization thereof)
     if(strcasecmp(extension,".js")==0)
    return "text/javascript";
    //Retrun text/x-php for any file whose path ends in .php (or any capitalization thereof)
     if(strcasecmp(extension,".php")==0)
    return "text/x-php";
    //Retrun image/png for any file whose path ends in .png (or any capitalization thereof)
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    */
    ...
    //this function is used to split the string with the delimit (space)
    //when it found space it return a pointer to the string before it 
